		...
		for (y = 0; y < l->ly; y++) {
			for (z = 0; z < l->lz; z++) {
				...
				if (l->obst[x][y][z] == false) {
					for (n = 0; n < length; n++) {
						l->node[x][y][z][v_1[0]] += t_1;
						l->node[x][y][z][v_2[0]] -= t_1;
					}
				}
			}
		}
	...
	for (x = 0; x < l->lx; x++) {
		for (y = 0; y < l->ly; y++) {
			for (z = 0; z < l->lz; z++) {
				...
				if (l->obst[x][y][z] == false) {
					for (n = 0; n < length; n++) {
						l->node[x][y][z][v_1[0]] += t_2;
						l->node[x][y][z][v_2[0]] -= t_2;
					}
				}				
			}
		}
...
/*void propagate(s_lattice *l) {
	...
	for (x = l->lx -1; x >= 0; x--) {
		for(y = 0; y < l->ly1; y++) {
			for(z = 0; z < l->lz; z++) {
			//Propagation in the x+ direction: 1, 2, 8, 9 and 14 and in the x- direction: 4, 5, 6, 11 and 12
			if (l->obst[x-1][y][z] == false) {
				l->node[x][y][z][1] = l->node[x-1][y][z][1];
				l->node[x][y+1][z][2] = l->node[x-1][y][z][2];
				l->node[x][y-1][z][8] = l->node[x-1][y][z][8];
				l->node[x][y][z+1][9] = l->node[x-1][y][z][9];
				l->node[x][y][z-1][14] = l->node[x-1][y][z][14];
			}
			if (l->obst[l->lx -x][y][z] == false) {
				l->node[l->lx -1 -x][y+1][z][4] = l->node[l->lx -x][y][z][4];
				l->node[l->lx -1 -x][y][z][5] = l->node[l->lx -x][y][z][5];
				l->node[l->lx -1 -x][y-1][z][6] = l->node[l->lx -x][y][z][6];
				l->node[l->lx -1 -x][y][z+1][11] = l->node[l->lx -x][y][z][11];
				l->node[l->lx -1 -x][y][z-1][12] = l->node[l->lx -x][y][z][12];
			}
			}

		}
	}
	//Propagation in the y-z plane y > 0: 3, 15 , 18, 7, 16 and 17
	for (x = 1; x < l->lx -1; x++) {
		for(y = l->ly -1; y > 0; y--) {
			for(z = 1; z < l->lz -1; z++) {
			if (l->obst[x][y-1][z] == false) {
				l->node[x][y][z][3] = l->node[x][y-1][z][3];
				l->node[x][y][z+1][15] = l->node[x][y-1][z][15];
				l->node[x][y][z-1][18] = l->node[x][y-1][z][18];
			}
			if (l->obst[x][l->ly -y][z] == false) {
				l->node[x][l->ly -1 -y][z][7] = l->node[x][l->ly -y][z][7];
				l->node[x][l->ly -1 -y][z+1][16] = l->node[x][l->ly -y][z][16];
				l->node[x][l->ly -1 -y][z-1][17] = l->node[x][l->ly -y][z][17];
			}
			}
		}
	}
	//Propagation in the y-z plane y = 0, z > 0: 10
	for (x = 1; x < l->lx -1; x++) {
		for(y = 1; y < l->ly -1; y++) {
			for(z = l->lz -1; z > 0; z--) {
			if (l->obst[x][y][z-1] == false) {

				l->node[x][y][z][10] = l->node[x][y][z-1][10];
			}
			if (l->obst[x][y][l->lz -z] == false) {
				l->node[x][y][l->lz -1 -z][13] = l->node[x][y][l->lz -z][13];
			}
			}
		}
	}
		...
		for(y = 1; y < l->ly -1; y++) {
			for(z = 1; z < l->lz -1; z++) {
				if (l->obst[x][y][z] == true) {
					for (n = 0; n < l->n; n++) {
						if(l->obst[x - l->e[n][0]] [y - l->e[n][1]] [z - l->e[n][2]] == false) {
							l->node[x - l->e[n][0]] [y - l->e[n][1]] [z - l->e[n][2]] [l->points[+1 - l->e[n][0]][+1 - l->e[n][1]][+1 - l->e[n][2]]] = l->node[x][y][z][n]; 
							l->node[x][y][z][n] = 0;
						}
					}
				}
		...
		for(y = 0; y < l->ly; y++) {
			for(z = 0; z < l->lz; z++) {
				if (l->obst[x][y][z] == false) {
					...
					u_x = l->temp[x][y][z][1] + l->temp[x][y][z][2] + l->temp[x][y][z][8] + l->temp[x][y][z][9] + l->temp[x][y][z][14]  - (l->temp[x][y][z][4] + l->temp[x][y][z][5] + l->temp[x][y][z][6] + l->temp[x][y][z][11] + l->temp[x][y][z][13]);
					...
					//for (i = 0; i < l->n; i++)
					//	n_equ[i] = d_loc * (A[i] + u_n[i] * B[i] + u_n[i] * u_n[i] * C[i] + u_squ * D[i]);
					n_equ[0] = d_loc * (l->A[0] + u_squ * l->D[0]);
					n_equ[1] = d_loc * (l->A[1] + u_n[1] * l->B[1] + u_n[1] * u_n[1] * l->C[1] + u_squ * l->D[1]);
					n_equ[2] = d_loc * (l->A[2] + u_n[2] * l->B[2] + u_n[2] * u_n[2] * l->C[2] + u_squ * l->D[2]);
					n_equ[3] = d_loc * (l->A[3] + u_n[3] * l->B[3] + u_n[3] * u_n[3] * l->C[3] + u_squ * l->D[3]);
					n_equ[4] = d_loc * (l->A[4] + u_n[4] * l->B[4] + u_n[4] * u_n[4] * l->C[4] + u_squ * l->D[4]);
					n_equ[5] = d_loc * (l->A[5] + u_n[5] * l->B[5] + u_n[5] * u_n[5] * l->C[5] + u_squ * l->D[5]);
					n_equ[6] = d_loc * (l->A[6] + u_n[6] * l->B[6] + u_n[6] * u_n[6] * l->C[6] + u_squ * l->D[6]);
					n_equ[7] = d_loc * (l->A[7] + u_n[7] * l->B[7] + u_n[7] * u_n[7] * l->C[7] + u_squ * l->D[7]);
					n_equ[8] = d_loc * (l->A[8] + u_n[8] * l->B[8] + u_n[8] * u_n[8] * l->C[8] + u_squ * l->D[8]);
					n_equ[9] = d_loc * (l->A[9] + u_n[9] * l->B[9] + u_n[9] * u_n[9] * l->C[9] + u_squ * l->D[9]);
					n_equ[10] = d_loc * (l->A[10] + u_n[10] * l->B[10] + u_n[10] * u_n[10] * l->C[10] + u_squ * l->D[10]);
					n_equ[11] = d_loc * (l->A[11] + u_n[11] * l->B[11] + u_n[11] * u_n[11] * l->C[11] + u_squ * l->D[11]);
					n_equ[12] = d_loc * (l->A[12] + u_n[12] * l->B[12] + u_n[12] * u_n[12] * l->C[12] + u_squ * l->D[12]);
					n_equ[13] = d_loc * (l->A[13] + u_n[13] * l->B[13] + u_n[13] * u_n[13] * l->C[13] + u_squ * l->D[13]);
					n_equ[14] = d_loc * (l->A[14] + u_n[14] * l->B[14] + u_n[14] * u_n[14] * l->C[14] + u_squ * l->D[14]);
					n_equ[15] = d_loc * (l->A[15] + u_n[15] * l->B[15] + u_n[15] * u_n[15] * l->C[15] + u_squ * l->D[15]);
					n_equ[16] = d_loc * (l->A[16] + u_n[16] * l->B[16] + u_n[16] * u_n[16] * l->C[16] + u_squ * l->D[16]);
					n_equ[17] = d_loc * (l->A[17] + u_n[17] * l->B[17] + u_n[17] * u_n[17] * l->C[17] + u_squ * l->D[17]);
					n_equ[18] = d_loc * (l->A[18] + u_n[18] * l->B[18] + u_n[18] * u_n[18] * l->C[18] + u_squ * l->D[18]);
				
					//relaxation step
					//Please. Open this loop!
					for (i = 0; i < l->n; i++) {
		...
		for(y = 0; y < l->ly; y++) {
			for(z = 0; z < l->lz; z++) {
				if (l->obst[x][y][z] == false) {
					...
					//x-, y- and z- velocity components
					u_x = (l->temp[x][y][z][1] + l->temp[x][y][z][2] + l->temp[x][y][z][8] + l->temp[x][y][z][9] + l->temp[x][y][z][14]  - (l->temp[x][y][z][4] + l->temp[x][y][z][5] + l->temp[x][y][z][6] + l->temp[x][y][z][11] + l->temp[x][y][z][13])) / d_loc;

					u_y = (l->temp[x][y][z][2] + l->temp[x][y][z][3] + l->temp[x][y][z][4] + l->temp[x][y][z][15] + l->temp[x][y][z][18]  - (l->temp[x][y][z][5] + l->temp[x][y][z][7] + l->temp[x][y][z][8] + l->temp[x][y][z][16] + l->temp[x][y][z][17])) / d_loc;
	...
	for(y = 0; y < l->ly; y++) {
		for(z = 0; z < l->lz; z++) {
			if (l->obst[x][y][z] == false) {
				...
				u_x = u_x + (l->node[x][y][z][1] + l->node[x][y][z][2] + l->node[x][y][z][8] + l->node[x][y][z][9] + l->node[x][y][z][14]  - (l->node[x][y][z][4] + l->node[x][y][z][5] + l->node[x][y][z][6] + l->node[x][y][z][11] + l->node[x][y][z][13])) / d_loc;
				n_free++;
			}
		}
	}
