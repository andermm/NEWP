...
void write_results(string file, s_lattice *l, double density) {
	...
	if (myid==0) {
		...
		MPI_Recv(&y,1,MPI_INT,myid-1,MPI_ANY_TAG,MPI_COMM_WORLD,&status);
	...
	for(y = 0; y < l->ly; y++) {
		for(x = l->lx_first; x < l->lx_last; x++) {
			...
			if (l->obst[x][y] == true) {
				...
				u_x = (l->node[x][y][2] + l->node[x][y][5] + l->node[x][y][6] - (l->node[x][y][4] + l->node[x][y][7] + l->node[x][y][8])) / d_loc;
	...
	fclose(archive);
	if (myid!=numprocs-1)
		MPI_Send(&x,1,MPI_INT,myid+1,1,MPI_COMM_WORLD);
}


//compute Reynolds number
void comp_rey(s_lattice *l, double omega, double density, double r_rey, int time) {
