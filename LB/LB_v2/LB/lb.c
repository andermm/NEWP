...
void alloc_lattice(s_lattice *l) {
	...
	for(x = 0; x < l->lx; x++) {
		l->node[x] = (double **) calloc(l->ly * l->n, sizeof(double*));
		l->temp[x] = (double **) calloc(l->ly * l->n, sizeof(double*));
		for(y = 0; y < l->ly; y++) {
...
void propagate(s_lattice *l) {
	...
	int aux;
	for (aux = l->lx_first-1; aux < l->lx_last+1; aux++) {
		//When aux = l->lx than x = 0 else aux = 0 than aux = l->lx - 1
		x = (aux+l->lx)%l->lx; 
...
void bounceback(s_lattice *l) {
	...
	for (x =  l->lx_first; x < l->lx_last; x++) {
		for(y = 0; y < l->ly; y++) {
			if (l->obst[x][y] == true) {
				//east
...
void comp_rey(s_lattice *l, s_properties *p, int time, double comm, double execution, int numprocs) {
	...
	FILE *c = fopen("comm.out", "a");
	FILE *e = fopen("exec.out", "a");
	fprintf(t, "%d %d %lg %lg %lg\n", numprocs, time, visc, vel, rey);
	fprintf(c, "%d %lg\n", numprocs, comm);
	fprintf(e, "%d %lg\n", numprocs, execution);
	fclose(t);
	fclose(c);
...
//////////////////////////////////////////
// Define a structure for the sublattices border to be send to the neighbors
//////////////////////////////////////////
